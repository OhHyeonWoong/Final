<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
	<head>
	<meta charset="UTF-8">
	<title>Insert title here</title>
	</head>
	<body>
	<%@ include file = "/WEB-INF/views/A8.Common/Header.jsp" %>	
		<div class="container">
		    <div class="row" style="padding-left: 40%;">
		        <div class="col-md-4">
		            <div class="panel panel-info">
		                <div class="panel-heading" align="center">
		                    <h3 class="panel-title"> 욱재 작업리스트</h3>
		                </div>
		                <ul class="list-group">
		                    <a href="#" class="list-group-item">작업 1 : </a>
		                    <a href="#" class="list-group-item">작업 2 : </a>
		                    <a href="#" class="list-group-item">작업 3 : </a>
		                    <a href="#" class="list-group-item">작업 4 : 구현중 </a>
		                    <a href="#" class="list-group-item">작업 5 : 구현중 </a>
		                    <a href="#" class="list-group-item">작업 6 : 구현중 </a>
		                    <a href="#" class="list-group-item">작업 7 : 구현중 </a>
		                    <a href="#" class="list-group-item">작업 8 : 구현중 </a>
		                    <a href="#" class="list-group-item">작업 9 : 구현중 </a>
		                    <a href="#" class="list-group-item">작업 10 : 구현중 </a>
		                    <a href="#" class="list-group-item">작업 11 : 구현중 </a>
		                    <a href="#" class="list-group-item">작업 12 : 구현중 </a>
		                </ul>
		            </div>
		        </div>
		    </div>
		</div>
	<%@ include file = "/WEB-INF/views/A8.Common/Footer.jsp" %>			
	</body>
</html>